...
class GameListSearchHolder(view: View): RecyclerView.ViewHolder(view) {
    ...
    fun bindValue(search: Search){
        GlobalScope.launch(Dispatchers.Main) {
            try {
                val response = withContext(Dispatchers.Default) {
                    NetworkManager2.getDetailsGame(search.appid)
                }
    ...
    fun clickOnButton(game: Search){
        button.setOnClickListener {
            (itemView.context as FragmentActivity).supportFragmentManager.beginTransaction()
                .replace(R.id.fragment_container, DescriptionGameActivity.DescriptionGameFragment.newInstance(game.appid))
